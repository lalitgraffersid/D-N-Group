...
class SubAdminDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $checkStatusAction = Role::where('name_slug','sub_admin')->whereRaw("find_in_set('".$status_action->id."',action_id)")->first();
        $roles = Role::where('name_slug','sub_admin')->first();
        ...
        return datatables()
            ...
            ->editColumn('status', function($row) use ($checkStatusAction,$checkStatusPermission) {
                if (!empty($checkStatusAction) && (!empty($checkStatusPermission) || Auth::user()->user_type == 'admin')) {
                    if($row->status == '1'){
                        return '<a href="'.route("sub_admin.status",$row->id).'" class="btn btn-success btn-sm" onclick="return confirm("Are you sure want to change status?")">Active</a>';
                    }
                    if($row->status == '0'){
                        return '<a href="'.route("sub_admin.status",$row->id).'" class="btn btn-danger btn-sm" onclick="return confirm("Are you sure want to change status?")">Inactive</a>';
                    }
                }else{
                    if($row->status == '1'){
            ...
            ->editColumn('action', function($row) use ($roles) {
                ...
                foreach ($action_ids as $key => $action_id) {
                    ...
                    if ($action->action_slug == 'edit' || $action->action_slug == 'delete' || $action->action_slug == 'view') {
                        $btn .= '<a href="'.route("sub_admin.$action->action_slug",$row->id).'" class="btn btn-'.$action->class.' btn-sm" data-placement="top" data-original-title="'.$action->action_title.'"><i class="'.$action->icon.'"></i>'.$action->action_title.'</a>&nbsp;';
                    }
                }
     ...
     */
    public function query(SubAdminDataTable $model)
    {
        //return $model->newQuery();
        $data = User::where('user_type','sub_admin')->select();
